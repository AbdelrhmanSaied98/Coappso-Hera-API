...
class crmController extends Controller
{
            ...
            $attendances = collect($attendances)->map(function($oneRecord) use ($countAll,$countPresent,$countAbsent)
            {
                ...
                return
                    [
                        ...
                        "beauty_center_name" => $attendance->employee->branch->beauty_center->name,
            ...
            $vacations = collect($vacationsArray)->map(function($oneRecord)
            {
                ...
                return
                    [
                        ...
                        "beauty_center_name" => $oneVacation->employee->branch->beauty_center->name,
            ...
            $finances = collect($financesArray)->map(function($oneRecord)
            {
                ...
                return
                    [
                        ...
                        "beauty_center_name" => $oneFinance->employee->branch->beauty_center->name,
            ...
            $allBooking = collect($allBooking)->map(function($oneRecord)
            {
                ...
                return
                    [
                        ...
                        "date" => $Booking->date,
                        "beauty_center_name" => $Booking->employee->branch->beauty_center->name,
    ...
    public function filterBookingCRM(Request $request,$monthName)
    {
        try {
            ...
            if ($request->beauty_center_name && $request->beauty_center_name != "")
            {
                $searchBooking = [];
                foreach ($allBooking as $id)
                {
                    ...
                    $name = $request->beauty_center_name;

                    $allBeauty_center = Beauty_center::where(function($query) use ($name) {
                        $query->where('name', 'LIKE','%'.$name.'%');
                    })->get();


                    foreach ($allBeauty_center as $oneBeauty)
                    {
                        foreach ($oneBeauty->branches as $branch)
                        {
